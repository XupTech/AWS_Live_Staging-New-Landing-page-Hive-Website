            ...
            if (err || !settingdata) {
                ...
                db.GetOneDocument('task', { _id: data.task }, {}, options, function (err, task) {
                    if (err) {
                        ...
                        if (duration_hours) {
                            if(duration_hours == 1) {
                                var momentHuman = duration_hours + " hr " + duration_minutes + " mins";
                            } else {
                        ...
                        invoice.worked_human_hours = duration_hours;
                        invoice.worked_human_mins = duration_minutes;
                            ...
                            if (err || !settingdata) {
                                ...
                                if (task.category.ratetype) {
                                    if (task.category.ratetype == "Flat") {
                                        ...
                                        invoice.amount.task_cost = parseFloat(provider_commision);
                                        invoice.amount.worked_hours_cost = parseFloat(invoice.amount.task_cost + (invoice.amount.task_cost * (Math.ceil(invoice.worked_hours) - 1))).toFixed(2);
                                        invoice.amount.total = parseFloat(invoice.amount.worked_hours_cost).toFixed(2);
                                    }
                                } 
                                            ...
                                            if (err) {
                                                ...
                                                db.GetDocument('emailtemplate', { name: { $in: ['Taskcompleteduser', 'Taskcompleted'] }, 'status': { $ne: 0 } }, {}, {}, function (err, template) {
                                                    if (err) {
                                                        ...
                                                        mailData.language = task.user.language;
                                                        mailData.html = [];
                                                        ...
                                                        mailData1.language = task.tasker.language;
                                                        mailData1.html = [];
                                                        ...
                                                        push.sendPushnotification(task.user._id, message, 'job_completed', 'ANDROID', notifications, 'USER', function (err, response, body) { }, task.tasker.username);
