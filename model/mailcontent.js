...
function sendmail(data, callback) {
    async.waterfall([
        ...
        function (settings, currencies, callback) {
            /*var query = {}
            if (data.language) {
               query = { name: data.template, 'lang': data.language, 'status': { $ne: 0 } }
            } else {
                query = { name: data.template, 'lang': 'en', 'status': { $ne: 0 } }
            }

            db.GetDocument('emailtemplate', query, {}, {}, function (err, template) {
                if (err || !template) { data.response = 'Unable to get email template'; res.send(data); }
                else { callback(err, settings, currencies, template); }
            });*/
            
            db.GetDocument('emailtemplate', { name: data.template, 'lang': { $in: [ data.language, 'en' ] }, 'status': { $ne: 0 } }, {}, {}, function (err, template) {
                if (err || template.length < 1) { data.response = 'Unable to get email template'; res.send(data); }
                else { 
                        if(template.length > 1){
                            if(template[0].lang == data.language) {
                                callback(err, settings, currencies, template[0]);
                            } else {
                                callback(err, settings, currencies, template[1]);
                            }
                        }
                        else{
                            callback(err, settings, currencies, template[0]);
        ...
        function (err, settings, currencies, template) {
            if (template.dispatch_mail) {
                var html = template.email_content;
                ...
                if (data.to) {
                    ...
                    var tomail = template.sender_email;
                }

                var mailOptions = {
                    from: template.sender_name + " <" + settings.settings.email_address + ">",
                    to: tomail,
