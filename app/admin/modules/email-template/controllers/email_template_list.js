...
function emailTemplateListCtrl(emailTemplateListResolve, EmailTemplateService, $modal, $scope,$stateParams) {
    ...
    var layout = [
        ...
        */
        {
            name: 'Actions',
            template: '<button class="btn btn-info btn-rounded btn-ef btn-ef-5 btn-ef-5b" ng-if="options.permission.edit != false" ui-sref=app.emailtemplate.action({action:"edit",id:content._id,page:currentpage,items:entrylimit,lang:"en",name:content.name})><i class="fa fa-edit"></i> <span>Edit</span></button>' +
    ...
    tlc.table.delete = {
        'permission': tlc.permission, service: '/email-template/delete', getData: function (currentPage, itemsPerPage, sort, search, status) {
            if (currentPage >= 1) {
                var skip = (parseInt(currentPage) - 1) * itemsPerPage;
                EmailTemplateService.getTemplateList(itemsPerPage, skip, sort, search, "en").then(function (respo) {
