...
function usersListCtrl(usersServiceResolve, UsersService, $scope, MainService, toastr, $stateParams, $rootScope) {
    ...
    if (usersServiceResolve[2]) {
        ...
        tlc.todayValue = usersServiceResolve[2].todayValue || 0;
    }

    $scope.statusValue = 0;
    tlc.statusPass = function statusPass(status, limit, skip) {
        $scope.statusValue = status;
        if (status == 1 || status == 2 || status == 0 || status == 4 || status == 5) {
    ...
    var layout = [
        ...
        },
        {
            ...
            '<button class="btn btn-danger btn-rounded btn-ef btn-ef-5 btn-ef-5b" ng-if="options.permission.delete != false && content.status != 0"  ng-click="CCC.openDeleteModal(small, content, options)" ><i class="fa fa-trash"></i> <span>Delete</span></button>'
        }
    ];
    tlc.table = {};
