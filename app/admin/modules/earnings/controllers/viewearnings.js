...
function viewearningsCtrl($scope, TaskerEarningReslove, toastr, EarningService, PaidEarningsServiceResolve, $stateParams, $rootScope) {
    ...
    if (encc.payDetails.total && encc.payDetails.total.extra_amount) {
    ...
    encc.paytasker = function paytasker() {
        ...
        var billing = encc.billingCycle.split('_');
        data.billing_cycle = moment(billing[0]).format('YYYY/MM/DD') + '-' + moment(billing[1]).format('YYYY/MM/DD');
        data.invoice = {};
