...
addTaskerCtrl.$inject = ['taskerAddServiceResolve', '$filter', '$state', 'TaskersService', '$scope', '$modal', 'toastr', 'CategoryServiceResolve', '$timeout', '$http', '$stateParams', 'NgMap', 'SettingsService', 'GeneralSettingsServiceResolve', '$rootScope', '$uibModal'];

function addTaskerCtrl(taskerAddServiceResolve, $filter, $state, TaskersService, $scope, $modal, toastr, CategoryServiceResolve, $timeout, $http, $stateParams, NgMap, SettingsService, GeneralSettingsServiceResolve, $rootScope, $uibModal) {
    ...
    atsc.settings = GeneralSettingsServiceResolve[0]
    if (atsc.radiusby == 'km') {
        atsc.radiusval = 1000;
    } else {
    ...
    atsc.showEditForm = function (docs) {
        $rootScope.docs = docs;
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: 'app/admin/modules/taskers/views/editdocument.modal.tab.html',
        ...
        modalInstance.result.then(function (docdata) {
            ...
            data.doc = docdata;
            TaskersService.taskerDocumentEdit(data).then(function (response) {
    ...
    if (taskerAddServiceResolve) {
        ...
        atsc.stripeConnect = function stripeConnect(user) {
            TaskersService.stripeConnect({ tasker: user }).then(function (respo) {
                if (respo.status == 1) {
                    atsc.tasker.stripe = respo.stripe;
                    toastr.success('Stripe connect ID has been generated sucessfully');
                } else {
                    toastr.error("Error in connecting Stripe");
                }
            });
        }
        ...
        if (atsc.tasker.working_days) {
            ...
            angular.forEach(atsc.tasker.working_days, function (selecteditem) {
                if (selecteditem.day && selecteditem.wholeday) {
                    count++;
                }
            });
        ...
        $scope.selectWholeweek = function (wholeweek) {
            if (wholeweek) {
                angular.forEach(atsc.tasker.working_days, function (selecteditem, key) {
                    selecteditem.selected = true;
                    selecteditem.wholeday = true;
                });
            } else {
                angular.forEach(atsc.tasker.working_days, function (selecteditem, key) {
                    selecteditem.selected = false;
                    selecteditem.wholeday = false;
                });
            }
        };
        ...
        $scope.changeWholeweek = function () {
            var count = 0;
            angular.forEach(atsc.tasker.working_days, function (selecteditem, key) {
                if (selecteditem.selected && selecteditem.wholeday) {
                    count++;
                }
            });
    ...
    atsc.submitTaskerEditData = function submitTaskerEditData(isValid) {
        ...
        if (calculate_age(atsc.tasker.birthdate.month, atsc.tasker.birthdate.date, atsc.tasker.birthdate.year, atsc.dob)) {
            ...
            // } 
            if (atsc.tasker.status == 1) { atsc.tasker.document_status = 1 }
            if (atsc.tasker.status != 1 && !atsc.tasker.stripe) {
                toastr.error('Please Stripe Connect This Tasker');
            } else {
                if (isValid) {
                    atsc.tasker.avatarBase64 = atsc.myCroppedImage;
                    atsc.tasker.username = atsc.tasker.firstname + ' ' + atsc.tasker.lastname;
                    console.log("Test-------------------------", atsc.tasker)
...
angular.module('handyforall.taskers').controller('DocumentModalInstanceCtrl', function ($scope, $rootScope, $state, toastr, TaskersService, $uibModalInstance, $translate) {
    var adcm = this;
    ...
    function loadFile(event) {
        ...
        if (adcm.docname.type == 'image/jpg' || adcm.docname.type == 'image/jpeg' || adcm.docname.type == 'image/png') {
            adcm.output.src = URL.createObjectURL(event.target.files[0]);
        } else if (adcm.docname.type == 'application/pdf') {
    ...
    function ok(valid) {
        if (valid) {
            ...
            toastr.error('Kindly Upload the Document');
        }
    };
