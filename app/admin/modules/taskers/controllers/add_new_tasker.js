...
addNewTaskerCtrl.$inject = ['$filter', '$state', '$scope', '$modal', 'toastr', '$timeout', 'TaskersService', 'taskerAddServiceResolve', 'TaskerDocumentResolve', 'CategoryServiceResolve', 'NgMap', 'SettingsService', 'GeneralSettingsServiceResolve'];

function addNewTaskerCtrl($filter, $state, $scope, $modal, toastr, $timeout, TaskersService, taskerAddServiceResolve, TaskerDocumentResolve, CategoryServiceResolve, NgMap, SettingsService, GeneralSettingsServiceResolve) {
    ...
    antsc.settings = GeneralSettingsServiceResolve[0];
    console.log("antsc.settings",antsc.settings)
    //antsc.settingstasker = antsc.settings.document_upload.status
    ...
    $scope.changeWholeweek = function () {
        ...
        if(count == 7) {
            antsc.wholeweek = true;
        } else {
            antsc.wholeweek = false;
        }
    }; 
    ...
    antsc.taskerdocument = TaskerDocumentResolve.data;
    antsc.fileNames = {}
    antsc.uploadDocument = function(valid, doc, name) {
        antsc.fileNames[name] = doc;
    }
    ...
    antsc.newsaveAvail = function () {
        ...
        if(antsc.tasker.working_days) {
            ...
            if(count == 0) {
                toastr.error('Please choose atleast one day');
            } else {
                angular.forEach(antsc.tasker.working_days, function (days) {
                    if(days.selected) {
                        if(!days.wholeday) {
                ...
                antsc.tasker.doc = []
                if (antsc.settings.document_upload.status == 1) {
    ...
    antsc.savedocuments = function () {
        TaskersService.taskerDocumentUpload(antsc.fileNames).then(function(resp) {
            if(resp.data.status == 1) {
                antsc.tasker.doc = resp.data.response.document;

                antsc.tasker1 = angular.copy(antsc.tasker);

                console.log("antsc.tasker", antsc.tasker);
