...
function AccountAvailabityCtrl($scope, $rootScope, $state, toastr, MainService, routes, $uibModal, $translate, $filter) {
    ...
    if (aac.user.role == 'tasker' && aac.user._id === userId) {
        ...
        $scope.selectWholeweek = function (wholeweek) {
            if (wholeweek) {
                angular.forEach(aac.user.working_days, function (selecteditem, key) {
        ...
        function saveAvailability() {
            ...
            if (aac.user.location.lat == '' || aac.user.location.lng == '') {
                ...
                MainService.getData(routes.availabity, aac.user).then(function(response) {
                    $translate('AVAILABILITY UPDATED SUCCESSFULLY').then(function(headline) { toastr.success(headline); }, function(translationId) { toastr.success(headline); });
                    $state.reload();
                }).catch(function(err) {
