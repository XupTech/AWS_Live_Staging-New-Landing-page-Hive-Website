...
function AccountHeaderCtrl($scope, $rootScope, $state, toastr, accountProfileResolve, seoSettingsResolve, settingsResolve, defaultCurrency, $cookieStore) {
    ...
    if ((currentUser === ahc.currentAcc._id && ahc.currentAcc.role === 'tasker') && ahc.settings.document_upload && ahc.settings.document_upload.status == 1) {
       $scope.menus.push({ "link": "account.documenttasker", "status": "active", "name": "Documents" })
    }
    // ----------------------------------------------------------------------------------------------
