...
function AccountTransactionCtrl($scope, $rootScope, $state, MainService, routes, toastr, $translate, $uibModal) {
    ...
    function taskUserTranscationViewModal(index, catid) {
        ...
        console.log("taskUserTransaction.invoice",taskUserTransaction.invoice)
        transcation.worked_hours = taskUserTransaction.invoice.worked_hours;
        if (taskUserTransaction.invoice.worked_human_hours) {
            transcation.worked_human_hours = taskUserTransaction.invoice.worked_human_hours;            
        }
        if (taskUserTransaction.invoice.worked_human_mins) {
            transcation.worked_human_mins = taskUserTransaction.invoice.worked_human_mins;
        }
        transcation.username = taskUserTransaction.user.username;
        transcation.address = taskUserTransaction.service_address;
...
function TaskUserTranscationViewModalCtrl($scope, task, $uibModalInstance, TaskDetails, defaultcurrency, getsettings, getmaincatname, MainService, routes) {
    ...
    if(ttemc.TaskDetails.date) {
        ttemc.TaskDetails.taskdate = moment(ttemc.TaskDetails.date).format(ttemc.getsettings.settings.date_format);
    }
