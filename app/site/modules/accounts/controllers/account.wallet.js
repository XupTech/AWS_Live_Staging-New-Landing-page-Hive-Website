...
function accountWalletCtrl($scope, $rootScope, toastr, $window, $stateParams, MainService, routes, $uibModal, $translate, $state) {
    ...
    function WalletRechargeByStripe(defaultCurrency) {
        ...
        if (data.amount && data.amount != 'NaN') {
            if (!(parseFloat(data.amount) >= $scope.walletMinAmt)) {
                $translate('PLEASE ENTER THE AMOUNT ABOVE' ).then(function (headline) { toastr.error(headline+ $scope.walletMinAmt); }, function (headline) { toastr.error(headline); });
            } else if (!(parseFloat(data.amount) <= $scope.walletMaxAmt)) {
                $translate('PLEASE ENTER THE AMOUNT BELOW').then(function (headline) { toastr.error(headline+ $scope.walletMaxAmt); }, function (headline) { toastr.error(headline); });
            } else {
                ...
                modalInstance.result.then(function (data) {
                    ...
                    MainService.getData(routes.walletRechargeStripe, { data: data, user: user }).then(function (response) {
                        if (response.status == 0) {
                            console.log("ddddddddddddddd",response.message)
...
function WalletRechargeModal($uibModalInstance, $state, $translate, Rechargeamount, toastr) {
    ...
    function ok(isValid) {
        if (isValid == true) {
            ...
            $translate('KINDLY ENTER THE CARD DETAILS').then(function (headline) { toastr.error(headline); }, function (headline) { toastr.error(headline); });
        }
    };
