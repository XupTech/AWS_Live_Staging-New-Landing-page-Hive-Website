...
function AccountCategoryCtrl($scope, $rootScope, $state, toastr, MainService, routes, $uibModal, $translate) {
    var acc = this;
    $scope.categoryButtonDisabled = false;
    ...
    function CategoryModal(category) {
        $scope.categoryButtonDisabled = true;
            ...
            MainService.getData(routes.addUpdateCat, selectedCategoryData).then(function (response) {
                if (response.result.nModified === 1) {
                    if (response.status == 'updated') {
                        $translate('CATEGORY UPDATED SUCCESSFULLY').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
                    } else {
                        $translate('ADDED SUCCESSFULLY').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
                    }
                }
                updateCat();
            }).catch(function (err) {
    ...
    function DeletecategoryModal(category, catname) {
        swal({
            title: $translate.instant('DELETE CATEGORY'),
            text: $translate.instant('ARE YOU SURE WANT TO DELETE THIS CATEGORY'),
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: $translate.instant('YES DELETE'),
...
function CategoriesModalInstanceCtrl($scope, $rootScope, $state, toastr, MainService, routes, experiences, defaultcurrency, user, categories, category, $uibModalInstance, $translate) {
    ...
    function onChangeCategoryChild(catchild) {
        ...
        MainService.getData(routes.catChild, { id: id }).then(function (response) {
            ...
            acm.selectedCategoryData.hour_rate = parseFloat((response.commision * acm.defaultcurrency.value).toFixed(2));
    ...
    function ok(valid) {
        if (valid) {
            ...
            $translate('PLEASE ENTER ALL CATEGORY FIELDS').then(function (headline) { toastr.error(headline); }, function (headline) { toastr.error(headline); });
        }
    };
