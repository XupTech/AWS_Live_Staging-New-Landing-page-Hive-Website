...
$cookieStore.remove('text');
  ...
  tpc.confirmatask = function confirmatask(message,hourlyprice) {
    if(tpc.user){
      swal({
            title: $translate.instant('CONFIRM AND BOOK'),
      ...
      TaskService.taskdetails($stateParams.task).then(function (response) {
        ...
        data.invoice = {
          'amount': {
            ...
            "task_cost": hourlyprice,
            "total": hourlyprice,
            "grand_total": hourlyprice
          }
        };
     ...
     swal({
          ...
          text: $translate.instant('TO PROCEED FURTHER PLEASE LOGIN OR SIGNUP'),
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          confirmButtonText: $translate.instant('YES CONFIRM'),
          cancelButtonText: $translate.instant('CANCEL')
            ...
            tpc.query ={};
            tpc.query.task =  tpc.task;
            $cookieStore.put("categeoryslug", tpc.query);
