            ...
            if (response.count <= 0) {
                ...
                if (tac.filter.about) {
                    ...
                    if (address[0]) {
                        ...
                        $translate('ADD TASK ADDRESS TO PROCEED').then(function (headline) { toastr.error(headline); }, function (headline) { toastr.error(headline); });
                    }
                } else {
                    $translate('TELL US ABOUT YOUR TASK').then(function (headline) { toastr.error(headline); }, function (headline) { toastr.error(headline); });
                }
            }
        }, function (error) {
            $translate('THE TASKER IS UN AVILABLE ON SELECTED ADDRESS').then(function (headline) { toastr.error(headline); }, function (headline) { toastr.error(headline); });
