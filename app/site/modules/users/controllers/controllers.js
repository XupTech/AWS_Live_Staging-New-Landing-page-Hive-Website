...
function userloginCtrl($scope, $http, $rootScope, $uibModal, $location, AuthenticationService, $state, toastr, $cookieStore, socket, PreviousState, $window, $translate, MainService, routes, $stateParams) {
    ...
    $scope.loginsocialstatus = 0;

    if($rootScope.loginsocialstatusroo){
         $scope.loginsocialstatus = 1;
    }
    ...
    ulgc.generateotp = function (isValid, type, phone) {
        if (isValid) {
            AuthenticationService.phonecheck(type, phone).then(function (phonecheck) {
                console.log("phonecheck", phonecheck);
                if (phonecheck.message == 'Account Inactivated') {
                    $translate('ACCOUNT DEACTIVATED').then(function (headline) { toastr.warning(headline); }, function (headline) { toastr.warning(headline); });
                } else if(phonecheck.message == 'Account Unverified') {
                    $translate('ACCOUNT UNVERIFIED').then(function (headline) { toastr.warning(headline); }, function (headline) { toastr.warning(headline); });
                } else if(phonecheck.message == 'Account Deleted') {
                    console.log("deleted");
                    $translate('ACCOUNT DELETED').then(function (headline) { toastr.warning(headline); }, function (headline) { toastr.warning(headline); });
                } else if(phonecheck.message == 'Account Not Exists') {
                    $translate('PLEASE ENTER REGISTERED MOBILE NUMBER').then(function (headline) { toastr.error(headline); }, function (headline) { toastr.error(headline); });
                } else if(phonecheck.message == 'Account Exists') {
                    var data = {};
                        ...
                        if (response) {
                            ...
                            if (type == 'user') {
                                ...
                                if (response.sms == 'development') { 
                                    ulgc.userotp = decrypted_otp.toString(CryptoJS.enc.Utf8);
                                }
                                ulgc.oldotp = decrypted_otp.toString(CryptoJS.enc.Utf8);
                                $translate('OTP SENT SUCESSFULLY').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
                            } else {
                                ulgc.showtaskerotpform = true;
                                if (response.sms == 'development') {
                                    ulgc.taskerotp = decrypted_otp.toString(CryptoJS.enc.Utf8);
                                }
                                ulgc.oldotp = decrypted_otp.toString(CryptoJS.enc.Utf8);
                                $translate('OTP SENT SUCESSFULLY').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
                            }
    ...
    ulgc.faceuseresndotp = function (isValid, type, phone) {
        if(isValid) {
             var loginsocialdata =  $cookieStore.get('socialloginData');
             type = loginsocialdata.role;
            AuthenticationService.phonecheck(type, phone).then(function (err, datadd) {
                if (err.message == 'Phone Number not exist') {
                   if(type == 'user'){

                        var data = {};
                        data.type = loginsocialdata.role;
                        data.code = phone.code;
                        data.phone = phone.number;
                        data.user_id = loginsocialdata._id;
                        MainService.generateotpface(data).then(function (response, err) {
                            if(response) {
                ...
                } else if (err.message == 'Phone Number Exist') {
    ...
    /*$scope.$watchCollection('loginsocialstatus', function (newval, oldval) {
        alert('watch');
    });*/



ulgc.socialresendotp = function(type) {
        if(type == 'user') {
            var data = {};
        ...
        MainService.generateotp(data).then(function (response, err) {
            if(response) {
                ...
                if(type == 'user') {
                    ...
                    //toastr.success('OTP Sent Successfully');
                    $translate('OTP RESENT SUCCESSFULLY').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
                } else {
                    ulgc.showtaskerotpform = true;
 ...
 ulgc.checkotpface = function (type, otp) {
      ...
      type = loginsocialdata.role;
        if(type == 'user') {
            if(ulgc.oldotp == otp) {
                ...
                console.log('ulgc.taskermobile',ulgc.taskermobile);
                ...
                MainService.getData(routes.userLogin, data).then(function (userdata, err) {
                   ...
                   var updatedata = {};
                   updatedata.user_id = loginsocialdata._id;
                   updatedata.type = 'user';
                   MainService.socialupdatelog(updatedata).then(function (updata, err) {
                    if(updata){
                        console.log("--->updata",updata);
                        AuthenticationService.SetCredentials(userdata.user, userdata.user_id, userdata.token, userdata.user_type, userdata.tasker_status, userdata.avatar);
                        localStorage.removeItem('TaskerData');
                        ...
                        $rootScope.$emit('webNotification', { user: userdata.user_id, type: userdata.user_type });
                             ...
                             delete $rootScope.loginsocialstatusroo;
                             $cookieStore.remove('socialloginData');
                        ...
                        // } else if($rootScope.currentState) {
                        //     $state.go($rootScope.currentState.name, $rootScope.currentparams, { reload: true });
                        // }
                        if ($cookieStore.get('task_details')) {
                            var tempdata = $cookieStore.get('task_details');
                            var tempdatas = $cookieStore.get('slug');
                            console.log("tempdata",tempdata);
                            //$cookieStore.remove('task_details');
                            //$cookieStore.remove('slug');

                            $state.go('hirestep1', { 'slug': tempdatas, task: JSON.stringify(tempdata) }, { reload: false });
                        } else if($rootScope.currentState) {
                            $state.go($rootScope.currentState.name, $rootScope.currentparams, { reload: true });
                        }
                        else {
                         ...
                         setTimeout(function(){ $scope.loginsocialstatus = 0; }, 3000);


                     }else{
                          toastr.error("Unable to update social login");
            ...
            if(ulgc.oldotp == otp) {
                ...
                data.firstname  = loginsocialdata.username;
                data.lastname   = loginsocialdata.username;
                data.email      =  loginsocialdata.email;
                localStorage.setItem('TaskerData', JSON.stringify(data));
    ...
    ulgc.facetaskersendotp = function (isValid, type, phone) {
        if(isValid) {
            var loginsocialdata =  $cookieStore.get('socialloginData');
    ...
    ulgc.checkotp = function (valid, type, otp, language) {
        if(valid) {
            if (type == 'user') {
                if (ulgc.oldotp == otp) {
                    var data = {};
                    ...
                    MainService.getData(routes.userLogin, data).then(function (userdata, err) {
                        if (err) {
                            $translate('UNABLE TO LOGIN').then(function (headline) { toastr.error(headline); }, function (headline) { toastr.error(headline); });
                        } else if(userdata.message) {
                            $translate(userdata.message).then(function (headline) { toastr.warning(headline); }, function (headline) { toastr.warning(headline); });
                        } else {
                    ...
                    $translate('THE ENTERED OTP IS INCORRECT').then(function (headline) { toastr.error(headline); }, function (headline) { toastr.error(headline); });
                }
            } else {
                if (ulgc.oldotp == otp) {
                    var data = {};
    ...
    $scope.facebookLogin = function () {
        var url = '/auth/facebook/typeuser',
            width = 1000,
            height = 650,
    ...
    $scope.facebookLogintasker = function () {
        var url = '/auth/facebook/typetasker',
            width = 1000,
            height = 650,
            top = (window.outerHeight - height) / 2,
            left = (window.outerWidth - width) / 2;
    ...
    $scope.googleLogintasker = function () {
        var url = '/auth/google/typetasker',
            width = 1000,
            height = 650,
            top = (window.outerHeight - height) / 2,
            left = (window.outerWidth - width) / 2;
    ...
    $window.app = {
        authState: function (data) {
            ...
            var  socialverify = data.socialverify;
            socialverify = socialverify.replace(/^"(.*)"$/, '$1');
            console.log("testt",socialverify);
            if(role == 'user'){
                if(socialverify && socialverify == 1){
                ...
                console.log('socialloginData 1',_id);

                  var socialloginData = {};
                 socialloginData.username  = username;
                 socialloginData._id  = _id;
                 socialloginData.token  = token;
                 socialloginData.role  = role;
                 $cookieStore.put('socialloginData', socialloginData);
                 console.log('socialloginData --->', $cookieStore.get('socialloginData'));
                 $scope.loginsocialstatus = 1;
                 $rootScope.loginsocialstatusroo = 1;
            ...
            }else
            {
                if(socialverify && socialverify == 1){
                    getdata = {};
                    getdata.taskerid = _id;
                    AuthenticationService.taskersociallogin(getdata).then(function (taskerdata, err) {
                        if(err) {
                            toastr.error('Unable to fetch data');
                        } else {
                            AuthenticationService.SetCredentials(taskerdata.user, taskerdata.user_id, taskerdata.token, taskerdata.user_type, taskerdata.tasker_status, taskerdata.avatar);
                            localStorage.removeItem('TaskerData');
                            $rootScope.$emit('notification', { user: taskerdata.user_id, type: taskerdata.user_type });
                            $rootScope.$emit('webNotification', { user: taskerdata.user_id, type: taskerdata.user_type });
                            if ($rootScope.currentState) {
                    ...
                    if ($rootScope.currentState) {
                     ...
                     if(first_name && first_name !=''){
                        socialloginData.firstname  = first_name;
                     }

                     if(lastname && lastname !=''){
                        socialloginData.lastname  = lastname;
                     }

                     socialloginData.email  = email;
                     socialloginData.role  = role;
                     $cookieStore.put('socialloginData', socialloginData);
                     console.log('socialloginData --->', $cookieStore.get('socialloginData'));
                     $scope.loginsocialstatus = 1;
...
registerCtrl.$inject = ['$scope', '$compile', '$rootScope', 'uiCalendarConfig', '$uibModal', 'CategoryserviceResolve', 'TaskerDocumentResolve', '$location', 'AuthenticationService', '$state', '$filter', 'toastr', '$cookieStore', '$stateParams', '$translate', 'accountService', 'NgMap', '$timeout', 'MainService', 'routes', 'settingsResolve', '$window'];

function registerCtrl($scope, $compile, $rootScope, uiCalendarConfig, $uibModal, CategoryserviceResolve, TaskerDocumentResolve, $location, AuthenticationService, $state, $filter, toastr, $cookieStore, $stateParams, $translate, accountService, NgMap, $timeout, MainService, routes, settingsResolve, $window) {
    var rgc = this;
    ...
    //rgc.settings = {}
    //rgc.settings.document_upload = {}
    rgc.type = $stateParams.type;
    rgc.settings = settingsResolve.settings;
    
    if (rgc.settings && rgc.settings.document_upload && rgc.settings.document_upload.status == 1) {
        rgc.categoryTab = '4. CATEGORY';
    }else{
        rgc.categoryTab = '3. CATEGORY';
    }
    ...
    console.log('$scope.privileges', $scope.privileges);
    ...
    $scope.selectWholeweek = function (wholeweek) {
        if (wholeweek) {
            angular.forEach(rgc.data.working_days, function (days, key) {
                days.selected = true;
                days.wholeday = true;
    ...
    $scope.changeWholeweek = function () {
        var count = 0;
        angular.forEach(rgc.data.working_days, function (days, key) {
            if(days.selected && days.wholeday) {
                count++;  
            } 
        });
    ...
    rgc.submitPersonalInfo = function (valid) {
        ...
        if (valid && rgc.checkTaskerDetail) {
            if(rgc.myCroppedImage != '') {
            ...
            AuthenticationService.checktaskeremail(rgc.data.email).then(function (err, data) {
    ...
    rgc.resetPersonalInfo = function() {
        if(rgc.myCroppedImage != '') {
            rgc.data.avatar = rgc.myCroppedImage;
            rgc.data.avatarBase64 = rgc.myCroppedImage;
        }
    ...
    rgc.taskerdocument = TaskerDocumentResolve.data;

    rgc.fileNames = {}
   
    rgc.uploadDocument = function(valid, doc, name) {
        console.log("doc",doc)
        rgc.fileNames[name] = doc;
    }
        ...
        if (rgc.data.working_days) {
            ...
            angular.forEach(rgc.data.working_days, function (days) {
                angular.forEach(days.slot, function (slot) {
                    if (days.selected & slot.selected || days.wholeday) {
                        count++;
                    }
                });
            });
            ...
            if(count == 0) {
                $translate('PLEASE SELECT ATLEAST ONE DAY').then(function (headline) { toastr.error(headline); }, function (headline) { toastr.error(headline); });
            } else {
                localStorage.setItem('TaskerData', JSON.stringify(rgc.data));

                if(rgc.settings.document_upload) {
                    if (rgc.settings.document_upload.status == 1) {
    ...
    rgc.submitDocumentsInfo = function (isValid) {
        if (isValid) {
            localStorage.setItem('TaskerData', JSON.stringify(rgc.data));
            $scope.tabs[3].active = true;
        } else {
    ...
    rgc.generateotp = function (isValid, type, phone) {
        if (isValid) {
            AuthenticationService.phonecheck(type, phone).then(function (err, data) {
                ...
                if (err.message == 'Account Exists' || err.message == 'Account Deleted') {
                    $translate('SORRY PHONE NUMBER ALREADY EXIST').then(function (headline) { toastr.error(headline); }, function (headline) { toastr.error(headline); });
                } else if (err.message == 'Account Not Exists') {
                    var data = {};
                    data.code = phone.code;
                    data.phone = phone.number;
                    ...
                    MainService.generateotp(data).then(function (response, err) {
                        if (response) {
                            ...
                            if (type == 'user') {
                                ...
                                if (response.sms == 'development') {
                                    rgc.userotp = decrypted_otp.toString(CryptoJS.enc.Utf8);
                                }
                                rgc.oldotp = decrypted_otp.toString(CryptoJS.enc.Utf8);
                                $translate('OTP SENT SUCESSFULLY').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
                            } else {
                                rgc.showtaskermobileform = false;
                                rgc.showtaskerotpform = true;
                                if (response.sms == 'development') {
                                    rgc.taskerotp = decrypted_otp.toString(CryptoJS.enc.Utf8);
                                }
                                rgc.oldotp = decrypted_otp.toString(CryptoJS.enc.Utf8);
                                $translate('OTP SENT SUCESSFULLY').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
                            }
    ...
    rgc.registerUser = function (isValid, language) {
        rgc.Error = '';
        var today = $filter('date')(new Date(), 'yyyy-MM-dd HH:mm:ss');

        rgc.UserDetails.today = today;
        rgc.UserDetails.role = rgc.type;
        rgc.UserDetails.language = rgc.language;
        ...
        if (isValid) {
            MainService.getData(routes.userRegister, rgc.UserDetails).then(function (userdata) {
                ...
                $translate('USER REGISTERED SUCESSFULLY').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
            }, function (err) {
    ...
    */
    ...
    rgc.TaskerRegister = function (valid, language) {
        ...
        if (valid > 0) {
            ...
            rgc.data.language = language;
            rgc.taskerData = angular.copy(rgc.data);
            rgc.taskerData.taskerfile = [];
            rgc.taskerData.taskerfile = $scope.surveyFiles;

            AuthenticationService.taskerDocumentUpload(rgc.fileNames).then(function(resp) {
                if(resp.data.status == 1) {
                    rgc.taskerData.doc = resp.data.response.document;
                    ...
                    if (err) {
                        ...
                        if (response == 'Email ID already exists') {
                            ...
                            if(response.status == 1) {
                                $translate('SINCE ITS DEMO ACCOUNT VERIFIED AUTOMATICALLY').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
                            } else {
                                $translate('TASKER REGISTERED SUCESSFULLY').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
                            }
                            $state.go('landing', {}, { reload: true });
                        }
                    }
