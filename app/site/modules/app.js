...
angular.module('handyforall.site', [
    ...
    .run(['$rootScope', '$state', '$location', '$cookieStore', '$http', '$stateParams', 'AuthenticationService', 'toastr', 'MainService', '$window', 'socket', 'ngMeta', '$cookieStore', '$transitions', 'tmhDynamicLocale', function ($rootScope, $state, $location, $cookieStore, $http, $stateParams, AuthenticationService, toastr, MainService, $window, socket, ngMeta, $cookieStore, $transitions, tmhDynamicLocale) {
        ngMeta.init();
        ...
        if ($rootScope.siteglobals.currentUser) {
            $rootScope.currAccAvatar = $rootScope.siteglobals.currentUser.avatar;
        ...
        $transitions.onStart({}, function (trans) {
            ...
            if (toState.name != 'login' && toState.name != 'signupotp' && toState.name != 'social' && toState.name != 'register' && toState.name != 'hirestep1' && toState.name != 'signup') {
                ...
                document.body.scrollTop = document.documentElement.scrollTop = 0;
                 // document.body.transition = document.documentElement.transition ="2s";
    ...
    .provider("routes", function routeProvider() {
        ...
        function restAPIs() {
            return ({
                ...
                "postheaderPosttask": { url: '/site/landing/postheader-posttask', method: 'POST' },
                ...
                "checkTaskerTaskStatus": { method: "POST", url: "/site/account/tasker/taskerTaskDetails" },
    ...
    .config(function (ngMetaProvider, ngIntlTelInputProvider, toastrConfig) {
        ...
        ngIntlTelInputProvider.set({ defaultCountry: 'in' });
    ...
    .config(function (tmhDynamicLocaleProvider) {
        ...
        tmhDynamicLocaleProvider.localeLocationPattern('/app/site/public/scripts/library/angular/i18n/angular-locale_{{locale}}.js');
    ...
    .config(['$stateProvider', '$urlRouterProvider', '$locationProvider', '$httpProvider', function ($stateProvider, $urlRouterProvider, $locationProvider, $httpProvider) {
        ...
        $stateProvider
            .state('landing', {
                ...
                resolve: {
                    MainserviceResolve: function (MainService, routes, $rootScope, $cookieStore) {
                        var language_code = $cookieStore.get('language_code') ? $cookieStore.get('language_code') : 'en';
                        //var languageUpdateValue = $cookieStore.get('language');
                        return MainService.getData(routes.landingPage, { 'lang' : language_code });
                    }
                }
            })
            ...
            .state('register', {
                ...
                resolve: {
                    ...
                    TaskerDocumentResolve: function (AuthenticationService) {
                        return AuthenticationService.taskerDocumentService();
                    }
                }
            })
            .state('becometasker', {
            ...
            .state('account.documenttasker', {
                url: '/documenttasker',
                action: 'list',
                controller: "AccountDocumentCtrl",
                controllerAs: 'ADTC',
                templateUrl: "app/site/modules/accounts/views/new-account/documenttasker.html",
                resolve: {
                    TaskerDocumentResolve: function (AuthenticationService, MainService, routes, $rootScope) {
                        var user = AuthenticationService.GetCredentials();
                    ...
                    TaskerTaskResolve: function (AuthenticationService, MainService, routes, $rootScope) {
                        var user = AuthenticationService.GetCredentials();
                        if (user.currentUser.user_type == 'tasker') {
                            var userId = $rootScope.userId;
                            return MainService.getData(routes.checkTaskerTaskStatus, { userId: userId });
                        }
                    }
                }
            })
    ...
    .controller('rootCtrl', function (config, $window, $scope, $rootScope, $state, AuthenticationService, MainService, socket, notify, $translate, toastr, $cookieStore, ngMeta, $sce, routes, tmhDynamicLocale) {
        ...
        var language_code = $cookieStore.get('language_code') ? $cookieStore.get('language_code') : 'en';
        MainService.getPostheaderPosttask(routes.postheaderPosttask, { 'lang' : language_code }).then(function (response) {
            rc.postHeader = response.response[0].PostHeader;
            rc.paymentprice = response.response[1].Paymentprice;
        });
        ...
        if ($cookieStore.get('Currency')) {
            ...
            $rootScope.defaultCurrency = $rootScope.currency;
        }
        ...
        rc.logout = function logout() {
            swal({
                ...
                text: $translate.instant('YOU WILL BE LOGOUT FROM THE SESSION'),
                type: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: $translate.instant('YES, LOGOUT'),
                cancelButtonText: $translate.instant('CANCEL')
            }).then(function (response) {
                var user = AuthenticationService.GetCredentials();
                AuthenticationService.Logout(user).then(function (data) {
                    AuthenticationService.ClearCredentials();
                    $translate('SUCCESSFULLY LOGED OFF').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
                    socket.emit('disconnect', { user: user.currentUser.user_id });
                    notify.emit('network disconnect', { user: user.currentUser.user_id });
        ...
        rc.setDefaultCurrency = function setDefaultCurrency(data) {
            MainService.getDefaultCurrency(data).then(function (response) {
                ...
                $rootScope.defaultCurrency = response[0];
                $rootScope.currency = response[0];
        ...
        MainService.getDefaultLanguage().then(function (response) {
            if ($cookieStore.get('language')) {
                ...
                rc.language_code = response[0].code;
        ...
        rc.setDefaultLanguage = function setDefaultLanguage(data) {
            MainService.getDefaultLanguage(data).then(function (response) {
                ...
                rc.language_code = response[0].code;
                tmhDynamicLocale.set(response[0].code);
                //$translate.proposedLanguage(response[0].code) ||
                $translate.use(response[0].code);
                //language
                
                MainService.getPostheaderPosttask(routes.postheaderPosttask, { 'lang' : rc.language_code }).then(function (response) {
                    rc.postHeader = response.response[0].PostHeader;
                    rc.paymentprice = response.response[1].Paymentprice;
                });
                if (rc.DefaultLanguage) {
                    MainService.getTransalatePage($rootScope.pageId, rc.DefaultLanguage, $rootScope.userId, $rootScope.usertype, response[0].code).then(function (response) {
        ...
        socket.on('webupdatechat', function (data) {
            ...
            if ($state.current.name != 'chat') {
                ...
                if (data.receiver == $rootScope.userId) {
                    ...
                    $translate("Message Received").then(function (headline) { toastr.info(headline); }, function (headline) { toastr.info(headline); });
                }
            }
        });
        ...
        notify.on('web notification', function (data) {
            ...
            if(data.message.action == 'account_status') {
                ...
                if(data.message.message == 'Your account status is changed to Inactive') {
                    var user = AuthenticationService.GetCredentials();
                    AuthenticationService.Logout(user).then(function (data) {
                        AuthenticationService.ClearCredentials();
                        $translate('SUCCESSFULLY LOGED OFF').then(function (headline) { toastr.success(headline); }, function (headline) { toastr.success(headline); });
            ...
            if(data.message.action == 'account_deleted') {
                setTimeout(function(){ 
                    var user = AuthenticationService.GetCredentials();
                    AuthenticationService.Logout(user).then(function (data) {
                        AuthenticationService.ClearCredentials();
            ...
            if(data.message.action == 'admin_notification') {
                value.currentUser.avatar = data.message.key1.avatar;
                value.currentUser.username = data.message.key1.firstname;
            ...
            $rootScope.$emit('webNotification', { user: $rootScope.userId, type: $rootScope.usertype });
            var tostr = toastr.info(data.message.message, 'Notification');
    ...
    .controller('MainCtrl', function ($scope, $location, $rootScope, $http, toastr, MainserviceResolve, MainService, $state, $translate, $cookieStore, swal, AuthenticationService, $cookieStore, ngMeta, $sce) {
        ...
        mac.banner_option = mac.banner[0].background_option;
        let videoUrl = mac.banner[0].video + '?start=60&rel=0&autoplay=1&mute=1&controls=0&showinfo=0&loop=1&iv_load_policy=3&enablejsapi=1';
        mac.banner_video = $sce.trustAsResourceUrl(videoUrl);
        // debugger
        console.log("mac.banner_video", mac.banner_video);
        ...
        console.log("mac.data.response[3]", mac.data.response[3]);
    ...
    .directive('defaultUserImage', function () {
        return {
            link: function (scope, element, attrs) {
                attrs.$observe('ngSrc', function (url) {
                    if (url === null || url === '') {
                        element.attr('src', 'app/site/public/images/avt.jpg');
                    } else {
                        element.bind('error', function () {
                            element.attr('src', 'app/site/public/images/avt.jpg');
                        });
