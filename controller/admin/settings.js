module.exports = function () {
    ...
    controller.currencyUSD = function (req, res) {
        db.GetOneDocument('currencies', { "code" : "USD", "name" : "Dollar" }, {}, {}, function (err, currencyDoll) {
            if (err) {
                res.send(err);
            } else {
                res.send(currencyDoll);
            }
        });
    }
    ...
    controller.currencyDefaultSave = function (req, res) {
        db.GetDocument('currencies',  { _id: req.body.id } , {}, {}, function (err, docdata) {
            if (err) {
                ...
                db.GetDocument('currencies',  {} , {}, {}, function (err, currencydata) {
                    if (err) {
                        res.send(err);
                    } else {                       

                        async.everySeries(currencydata, function (currencydata, callback) {                           
                            if(currencydata._id == req.body.id){                          
                                var def = 1;
                                var curvalue = currencydata.value / docdata[0].value
                            }else{
                                var def = 0;
                                var curvalue = currencydata.value / docdata[0].value
                            }

                            db.UpdateDocument('currencies', { _id: currencydata._id }, { default: def, value: curvalue }, function (err, docdata) {
                                if (err) {
                                    res.send(err);
                                } else {
        ...
        //     } else {
        //         db.UpdateDocument('currencies', { _id: req.body.id }, { default: 1, value: 1 }, function (err, docdata) {
        //             if (err) {
        //                 res.send(err);
        //             } else {
        //                 res.send(docdata);
                        ...
                        if (error) {
                            ...
                            fs.writeFile(path.join(__dirname, '../../', '/uploads/languages/' + req.body.code + '.json'), JSON.stringify(objectValue, null, 4), function (err, respo) {
                                if (err) {
                                    ...
                                    fs.readFile(path.join(__dirname, '../../', '/uploads/languages/' + docdata.code + '_mob.json'), "utf8", function (error, objectdata) {
                                        if (error) {
                                            res.send(err);
                                        }
                                        else {
                                            obj = JSON.parse(objectdata)
                                            objectValue = obj;
                                            fs.writeFile(path.join(__dirname, '../../', '/uploads/languages/' + req.body.code + '_mob.json'), JSON.stringify(objectValue, null, 4), function (err, respo) {
                                                if (err) {
                                                    res.send(err);
                                                } else {
                                                    db.UpdateDocument('languages', { _id: { $in: req.body._id } }, data, function (err, result) {
                ...
                if (err) {
                    ...
                    if (docdata != null) {
                        ...
                        async.parallel([
                            ...
                            function (callback) {
                                fs.readFile(path.join(__dirname, '../../', '/uploads/languages/en_mob.json'), "utf8", function (error, objectdata) {
                                    if (error) {                             
                                        obj = JSON.parse(objectdata)
                                        objectValue = obj;
                                        fs.writeFile(path.join(__dirname, '../../', '/uploads/languages/' + req.body.code + '_mob.json'), JSON.stringify(objectValue, null, 4), function (err, respo) {
    ...
    controller.save = function (req, res) {
        ...
        data.settings.wallet.amount.minimum = parseInt(req.body.wallet.amount.minimum);
        data.settings.wallet.amount.maximum = parseInt(req.body.wallet.amount.maximum);
        data.settings.wallet.amount.welcome = parseInt(req.body.wallet.amount.welcome);
        ...
        var document_upload = {};
        data.settings.document_upload = req.body.document_upload;

        var referral = {};
        data.settings.referral = req.body.referral;
        data.settings.referral.amount.referral = parseInt(req.body.referral.amount.referral);
        data.settings.referral.amount.referrer = parseInt(req.body.referral.amount.referrer);

        data.settings.categorycommission = req.body.categorycommission;

        data.settings.map_api = req.body.map_api;

        data.settings.resttime = {};
        data.settings.resttime.option = req.body.resttime;
        data.settings.resttime.status = req.body.resttimestatus;
        ...
        db.UpdateDocument('tasker', { 'status': {$ne: 0} },{'radius':data.settings.tasker_radius}, {  multi: true }, function (err, response) {
        });

        db.UpdateDocument('settings', { "alias": "general" }, data, { upsert: true }, function (err, response) {
            if (err || response.nModified == 0) {
                res.send(err);
            } else {
                GLOBAL_CONFIG = data.settings; // For Testing
                /*
    ...
    controller.filesave = function (req, res) {
        ...
        db.GetOneDocument('settings', { alias: 'mobile' }, {}, {}, function (err, docdata) {
            if (err || !docdata) {
                ...
                docdata.settings.apns.mapuserios = req.body.mapuserios;
                docdata.settings.apns.maptaskerios = req.body.maptaskerios;
                docdata.settings.gcm.mapuserandroid = req.body.mapuserandroid;
                docdata.settings.gcm.maptaskerandroid = req.body.maptaskerandroid;
    ...
    controller.languageDownload = function (req, res) {
        req.checkBody('code', 'Invalid Data').notEmpty();
        ...
        if(req.body.type == 'web') {
            file_path = '/uploads/locales/' + req.body.code + '_mob.json';
            req.body.code = 'en_mob';
        } else {
            file_path = '/uploads/locales/' + req.body.code + '.json';
            req.body.code = 'en';
        }
        ...
        fs.readFile(path.join(__dirname, '../../', file_path), "utf8", function (error, objectdata) {
            if (error) {
                data.response = err;
                res.send(data);
            } else {
                var tempobj = JSON.parse(objectdata)
                var keys = Object.keys(tempobj);

                var xml = builder.create('resources', { encoding: 'utf-8' }).att('xmlns:xliff', "urn:oasis:names:tc:xliff:document:1.2")
                for (var i = 0; i < keys.length; i++) {
                    xml.ele('string', { name: keys[i] }, tempobj[keys[i]])
                }
                xml.end({ pretty: true });
                fs.writeFile(path.join(__dirname, '../../', '/uploads/languages/mobilelanguagecontent.xml'), xml, function (err, respo) {
                    if (err) {
                        data.response = err;
                        res.send(data);
                    } else {
                        data.status = 1;
                        data.response = "uploads/languages/mobilelanguagecontent.xml";
                        res.send(data);
                    }
                });
            }
    ...
    controller.languageAdd = function (req, res) {
        console.log("req.body", req.body);
        req.checkBody('language', 'Invalid Data').notEmpty();
        ...
        if (language.length > 0) {
            async.each(language, function (lang, callback) {
                fs.readFile(path.join(__dirname, '../../', '/uploads/locales/' + lang.code + '.json'), "utf8", function (error, objectdata) {
                    if (error) {
                        callback(error);
                    } else {
                        var obj = JSON.parse(objectdata)
                        obj[lang.key] = lang.value;
                        var finaldata = obj;
                        fs.writeFile(path.join(__dirname, '../../', '/uploads/locales/' + lang.code + '.json'), JSON.stringify(finaldata, null, 4), function (err, respo) {
                            if (err) {
                                callback(err);
                            } else {
                                callback(err, respo)
                ...
                if (err) {
                    ...
                    data.status = 1;
                    data.response = "Languages Added Successfully";
                    res.send(data);
                }
            })
        } else {
            data.response = "Empty Data";
            res.send(data);
        }
    };

    controller.languageWrite = function (req, res) {
        req.checkBody('language', 'Invalid Data').notEmpty();
        ...
        if (language.length > 0) {
            async.each(language, function (lang, callback) {
                fs.readFile(path.join(__dirname, '../../', '/uploads/languages/' + lang.code + '.json'), "utf8", function (error, objectdata) {
                    if (error) {
                        callback(err);
                    } else {
                        var obj = JSON.parse(objectdata)
                        obj[lang.key] = lang.value;
                        var finaldata = obj;
                        fs.writeFile(path.join(__dirname, '../../', '/uploads/languages/' + lang.code + '.json'), JSON.stringify(finaldata, null, 4), function (err, respo) {
                            if (err) {
                                callback(err);
                            } else {
                                fs.readFile(path.join(__dirname, '../../', '/uploads/locales/' + lang.code + '.json'), "utf8", function (error, objectdata1) {
                                    if (error) {
                                        callback(err);
                                    } else {
                                        var obj1 = JSON.parse(objectdata1)
                                        obj1[lang.key] = lang.value;
                                        var finaldata1 = obj1;
                                        fs.writeFile(path.join(__dirname, '../../', '/uploads/locales/' + lang.code + '.json'), JSON.stringify(finaldata1, null, 4), function (err, respo1) {
                                            if (err) {
                                                callback(err);
                                            } else {
                                                callback(err, respo1)
    ...
    controller.multideletelanguageDetails = function (req, res) {
        console.log("Req.Body", req.body);
        var data = {};
        data.status = 0;
        req.checkBody('del', 'Invalid Data').notEmpty();
        req.checkBody('language', 'Invalid Data').notEmpty();
        var errors = req.validationErrors();
        if (errors) {
            data.response = errors[0].msg;
        ...
        var del = req.body.del, language = req.body.language;
        if (del.length > 0 && language.length > 0) {
            async.each(language, function (lang, callback) {
                fs.readFile(path.join(__dirname, '../../', '/uploads/languages/' + lang.code + '.json'), "utf8", function (error, languageRaw) {
                    if (error) {
                        callback(error);
                    } else {
                        var obj = JSON.parse(languageRaw);
                        var newarray = obj;
                        del.forEach(function(clear){
                            var remove = clear.key
							delete newarray[remove];
                        })
                        console.log("Lang Code",lang.code);
                        fs.writeFile(path.join(__dirname, '../../', '/uploads/languages/' + lang.code + '.json'), JSON.stringify(newarray, null, 4), function (error, respo) {
                            if (error) {
                                console.log("Failed");
                                callback(error);
                            } else {
                ...
                if (err) {
                    ...
                    console.log("Success",respo);
                    data.status = 1;
                    data.response = "Language Content Deleted Successfully";
                    res.send(data);
                }
            })
        } else {
    ...
    controller.deletelanguageDetails = function (req, res) {
        console.log("Req.Body", req.body);
        ...
        if (language.length > 0) {
            async.each(language, function (lang, callback) {
                fs.readFile(path.join(__dirname, '../../', '/uploads/locales/' + lang.code + '.json'), "utf8", function (error, languageRaw) {
                    if (error) {
                        callback(error);
                    } else {
                        var obj = JSON.parse(languageRaw);
                        ...
                        for (var i = 0; i < keys.length; i++) {
                            if (keys[i] != del) {
                                if (obj[keys[i]]) {
                                    newarray[keys[i]] = obj[keys[i]];
                                } else {
                                    newarray[keys[i]] = '';
                                }
                            }
                        }
                        fs.writeFile(path.join(__dirname, '../../', '/uploads/locales/' + lang.code + '.json'), JSON.stringify(newarray, null, 4), function (error, respo) {
                            if (error) {
                                console.log("Failed");
                                callback(error);
                            } else {
    ...
    controller.getmobilelanguageDetails = function (req, res) {
       console.log("Req.Body", req.body);
       ...
       fs.readFile(path.join(__dirname, '../../', file_path), "utf8", function (error, englishRaw) {
           if (error) {
               res.send(error);
           } else {
               var english = JSON.parse(englishRaw);
               var engKeys = Object.keys(english);                
               fs.readFile(path.join(__dirname, '../../', file_path), "utf8", function (error, languageRaw) {
                   if (error) {
                       res.send(error);
                   } else {
                       var obj = JSON.parse(languageRaw);
                       ...
                       var keysasdasds = Object.keys(newarray);
                       if (req.body.search) {
                           var searchData = keysasdasds.filter(function (seek) {
                               if (seek.includes(req.body.search)) {
                                   return seek;
                               }
                           })
                           var languageManage = {};
                           languageManage.total = searchData.length;
                           languageManage.data = {};
                           var start = req.body.limit * (req.body.current - 1);
                           var end = req.body.limit + (req.body.limit * (req.body.current - 1));
                           for (var i = start; i < end; i++) {
                               languageManage.data[searchData[i]] = newarray[searchData[i]];
                           }
                           res.send(languageManage);
                       } else {
                           var languageManage = {};
    ...
    controller.languagemobileSaveTranslation = function (req, res) {
        req.checkBody('id', 'Invalid Id').notEmpty();
        req.checkBody('data', 'Invalid Data').notEmpty();
        var errors = req.validationErrors();
        if (errors) { res.send(errors, 400); return; }
        ...
        var file_path = '';
        if(req.body.type == 'web') {
            file_path = '/uploads/locales/' + req.body.id + '_mob.json';
            req.body.id = 'en_mob';
        } else {
            file_path = '/uploads/locales/' + req.body.id + '.json';
            req.body.id = 'en';
        }
            ...
            if (error) {
                ...
                fs.readFile(path.join(__dirname, '../../', file_path), "utf8", function (error, objectdata) {
                    if (error) {
                        ...
                        var outputfile = path.join(__dirname, "../../", file_path);
                        fs.writeFile(outputfile, JSON.stringify(newarray, null, 4), function (err) {
                            if (err) {
                                res.send(err);
                            } else {
