...
module.exports = function (io) {
	...
	controller.list = function (req, res) {
		...
		var query = {}
		if (req.body.lang) {
			query = { status: { $ne: 0 }, lang: req.body.lang }
		}else{
			query = { status: { $ne: 0 } }
		}
		var emailQuery = [{
	...
	controller.getTemplateByName = function (req, res) {
		db.GetOneDocument('emailtemplate', { 'name': req.body.name, 'lang':req.body.lang, status: { $ne: 0 } }, {}, {}, function (err, docdata) {
			if (err || !docdata) {
				res.send({"status": "0","msg": "Template Not Found"});
			} else {
				res.send({"status": "1","response": docdata});
			}
		});
	};
	...
	controller.save = function (req, res) {
		...
		if (errors) {
			...
			if (req.body._id) {
				...
				db.GetDocument('emailtemplate', { 'name': req.body.name, 'lang':req.body.lang, status: { $ne: 0 } }, {}, {}, function (err, existTemplate) {
					if (err) {
						res.send(err);
					} else if(existTemplate.length > 0){
						res.send({"status": "422","msg":"Template Already Exist"});
					}else{
