...
module.exports = function () {
        ...
        if (req.body._id) {
            db.UpdateDocument('category', { _id: req.body._id }, req.body, { multi: true }, function (err, docdata) {
                if (err) {
                    ...
                    if(req.body.parent) {
                        db.UpdateDocument('tasker', { 'taskerskills.childid': req.body._id }, { $set: { "taskerskills.$.hour_rate": req.body.commision } }, {}, function (err, result) {
                            if(err) {
                            } else {
                                res.send(docdata);
                            }
    ...
    controller.updatePriority = function (req, res) {
        if(req.body.priority == 1) {
            db.GetCount('category', { 'priority': 1 }, function (err, count) {
                if (err) {
                    res.send(err);
                } else {
                    if(count > 4) {
                        ...
                        db.UpdateDocument('category', { _id: req.body.catid }, { 'priority': req.body.priority }, { multi: true }, function (err, docdata) {
                            if(err) {
                                res.send(err);
                            } else {
                                res.send({'status': 'Success', 'priority': req.body.priority});
                            }
                        });
                    }
                }
