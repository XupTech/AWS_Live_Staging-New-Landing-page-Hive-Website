module.exports = function (app, io) {
    ...
    router.userlogin = function (req, res) {
        ...
        db.GetOneDocument('users', { 'phone.code': req.body.phone.code, 'phone.number': req.body.phone.number, 'status': 1 }, {}, {}, function (err, user) {
            if (err) {
                res.send(err);
            } else {
                if (user) {
    ...
    router.taskersociallogin = function (req, res) {

        req.checkBody('taskerid', 'Tasker id is required').notEmpty();
        var errors = req.validationErrors();
        var data = {}
        data.status = "0";
        if (errors) { data.message = errors[0].msg; res.send(data); return; }

        db.GetOneDocument('tasker', { '_id':new mongoose.Types.ObjectId(req.body.taskerid), 'status': { $in: [1, 2, 3] } }, {}, {}, function (err, user) {
            if (err) {
                res.send(err);
            } else if (!err && !user) {
                res.send({ message: 'Phone not exists' });
                 ...
                 var authHeader = jwt.sign(user.phone.number, CONFIG.SECRET_KEY);
                ...
                db.UpdateDocument('tasker', { _id: user._id }, data, {}, function (err, docdata) {
                    if (err) {
                        ...
                        data.phone = user.phone.number;
                        data.user_type = user.role;
                        data.verify_status = user.status;
                        data.avatar = user.avatar;
                        res.send(data);
                    }
                }); 
            ...
            db.GetOneDocument('users', { "phone.number": req.body.phone.number }, {}, {}, function (err, pdocdata) {
                if (err) {
                    ...
                    if (pdocdata && pdocdata.phone.code == req.body.phone.code && pdocdata.phone.number == req.body.phone.number) {
                        res.status(400).send('SORRY PHONE NUMBER ALREADY EXIST');
                    } else {
                                    ...
                                    db.GetOneDocument('users', { 'email': req.body.email.toLowerCase() }, {}, {}, function (err, users) {
                                        if (err) {
                                            ...
                                            if (users && users.email == req.body.email.toLowerCase()) {
                                                res.status(400).send('SORRY EMAIL ID ALREADY EXIST');
                                            }
