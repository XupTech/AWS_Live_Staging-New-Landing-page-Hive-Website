module.exports = function (io,i18n) {
        ...
        db.GetOneDocument('walletReacharge', { "user_id": req.body.userid }, {}, {}, function (err, wallet) {
            if (err) {
                ...
                if (wallet) {
                    if (wallet.total == 0) {
                        res.send({ status: 0, message: 'Recharge Your Wallet' });
                    } else if ((wallet.total < req.body.amount) && wallet.total != 0) { //Partial Payment
                                                            ...
                                                            if (err) {
                                                                ...
                                                                db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
                                                                    if (err) {
                                                                        ...
                                                                        var user_lang = docdata.user.language || 'en';
                                                                        db.GetDocument('emailtemplate', { name: 'PaymentDetailstoUser', 'status': { $ne: 0 }, 'lang': { $in: [ user_lang, 'en' ] } }, {}, {}, function (err, template) {
                                                                            if (err) {
                                                                                ...
                                                                                var transmode = i18n.__(PaymentMode);
                                                                                ...
                                                                                if(docdata.invoice.amount.coupon && docdata.invoice.amount.extra_amount) {
                                                                                    html1 = html1.replace(/{{totalamount}}/g, currencies.symbol + ' ' + (docdata.invoice.amount.total + docdata.invoice.amount.extra_amount + docdata.invoice.amount.service_tax - docdata.invoice.amount.coupon).toFixed(2));
                                                                                } else if(docdata.invoice.amount.coupon && !docdata.invoice.amount.extra_amount) {
                                                                                    html1 = html1.replace(/{{totalamount}}/g, currencies.symbol + ' ' + (docdata.invoice.amount.total + docdata.invoice.amount.service_tax - docdata.invoice.amount.coupon).toFixed(2));
                                                                                } else if(!docdata.invoice.amount.coupon && docdata.invoice.amount.extra_amount) {
                                                                                    html1 = html1.replace(/{{totalamount}}/g, currencies.symbol + ' ' + (docdata.invoice.amount.total + docdata.invoice.amount.extra_amount + docdata.invoice.amount.service_tax).toFixed(2));
                                                                                } else {
                                                                                ...
                                                                                html1 = html1.replace(/{{mode}}/g, transmode);
                                                                        ...
                                                                        var tasker_lang = docdata.tasker.language || 'en';
                                                                        db.GetDocument('emailtemplate', { name: 'PaymentDetailstoTasker', 'status': { $ne: 0 }, 'lang': { $in: [ tasker_lang, 'en' ] } }, {}, {}, function (err, templates) {
                                                                            if (err) {
                                                                                ...
                                                                                if(docdata.payment_type == 'wallet') {
                                                                                    PaymentMode =  i18n.__('Wallet');
                                                                                }
                                                                                
                                                                                var text1 = i18n.__("Please Download the attachment to see your Invoice details");
                                                                                var text2 = '<b>'+i18n.__('Please Download the attachment to see your Invoice details')+'</b>';
                            ...
                            if (err) {
                                ...
                                db.GetOneDocument('task', { "_id": req.body.taskid }, {}, {}, function (err, task) {
                                    if (err) {
                                        ...
                                        var balanceamount = {};
                                        if(task.invoice.amount.balance_amount > req.body.amount) {
                                            balanceamount = parseFloat(task.invoice.amount.balance_amount) - parseFloat(req.body.amount);
                                        } else {
                                            balanceamount = parseFloat(task.invoice.amount.balance_amount);
                                        }
                                                                                    ...
                                                                                    if (err) {
                                                                                        ...
                                                                                        db.GetDocument('emailtemplate', { name: 'PaymentDetailstoUser', 'status': { $ne: 0 }, 'lang': { $in: [ user_lang, 'en' ] } }, {}, {}, function (err, template) {
                                                                                            if (err) {
                                                                                                ...
                                                                                                if(docdata.payment_type == 'wallet') {
                                                                                                   var tPaymentMode = i18n.__('Wallet');
                                                                                                }
                                                                                                ...
                                                                                                html1 = html1.replace(/{{Servicetax}}/g, currencies.symbol + docdata.invoice.amount.service_tax.toFixed(2));
                                                                                                html1 = html1.replace(/{{mode}}/g, tPaymentMode);
                                                                                        ...
                                                                                        db.GetDocument('emailtemplate', { name: 'PaymentDetailstoTasker', 'status': { $ne: 0 }, 'lang': { $in: [ tasker_lang, 'en' ] } }, {}, {}, function (err, templates) {
                                                                                            if (err) {
                                                                                                ...
                                                                                                docdata.payment_type = i18n.__(docdata.payment_type);
                    ...
                    if (err) {
                        ...
                        db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
                            if (err) {
                                ...
                                mailData2.language = docdata.tasker.language;
                                mailData2.html = [];
                                ...
                                mailData3.language = docdata.user.language;
                                mailData3.html = [];
    ...
    query = { 'status': { "$in": status }, 'user': new mongoose.Types.ObjectId(req.body._id) }

    if (req.body.taskfrom) {
        //var start = moment(new Date(req.body.taskfrom)).format('YYYY-MM-DD') + ' 00:00:00';
        //var fromDate = timezone.tz(start, results.settings.time_zone); 
        var fromDate = req.body.taskfrom;
        query = { 'status': { "$in": status }, 'user': new mongoose.Types.ObjectId(req.body._id),'booking_information.booking_date': { '$gte': new Date(fromDate) }}
    }
    if (req.body.taskto) {
        var end = moment(new Date(req.body.taskto)).format('YYYY-MM-DD') + ' 23:59:59';
        //var toDate = timezone.tz(end, results.settings.time_zone);
        var toDate = end;
        query = { 'status': { "$in": status }, 'user': new mongoose.Types.ObjectId(req.body._id),'booking_information.booking_date': { '$lte': new Date(toDate) }}
    }
    if (req.body.taskfrom && req.body.taskto) {
        query = { 'status': { "$in": status }, 'user': new mongoose.Types.ObjectId(req.body._id),'booking_information.booking_date': { '$gte': new Date(fromDate), '$lte': new Date(toDate) } }
    }
...
controller.getSearchTaskList = function getSearchTaskList(req, res) {
        ...
        var sorting = {};
        sorting[request.sortby] = -1;
        db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, results) {
            if (err || !results) {
                data.response = 'Configure your website settings';
                res.send(data);
            } else {
                ...
                if (req.body.jobid.length > 4) {
                    if (req.body.taskfrom && req.body.taskto) { // job id, from_date and to_date
                        match_query = { 'status': { "$in": status }, 'booking_id': req.body.jobid, 'updatedAt': { '$gte': new Date(fromDate), '$lte': new Date(toDate) }, 'user': new mongoose.Types.ObjectId(req.body._id) };
                    } else if (req.body.taskfrom && !req.body.taskto) { // job id and from_date
                        match_query = { 'status': { "$in": status }, 'booking_id': req.body.jobid, 'updatedAt': { '$gte': new Date(fromDate) }, 'user': new mongoose.Types.ObjectId(req.body._id) };
                    } else if (req.body.taskto && !req.body.taskfrom) { // job id and to_date
                        match_query = { 'status': { "$in": status }, 'booking_id': req.body.jobid, 'updatedAt': { '$lte': new Date(toDate) }, 'user': new mongoose.Types.ObjectId(req.body._id) };
                    } else { // job id
                    ...
                    if (req.body.taskfrom && req.body.taskto) { // from_date and to_date
                        match_query = { 'status': { "$in": status }, 'updatedAt': { '$gte': new Date(fromDate), '$lte': new Date(toDate) }, 'user': new mongoose.Types.ObjectId(req.body._id) };
                    } else if (req.body.taskfrom && !req.body.taskto) { // from_date
                        match_query = { 'status': { "$in": status }, 'updatedAt': { '$gte': new Date(fromDate) }, 'user': new mongoose.Types.ObjectId(req.body._id) };
                    } else if (req.body.taskto && !req.body.taskfrom) { // to_date
                        match_query = { 'status': { "$in": status }, 'updatedAt': { '$lte': new Date(toDate) }, 'user': new mongoose.Types.ObjectId(req.body._id) };
                    }
                }
    ...
    controller.getTaskDetailsByStaus = function getTaskDetailsByStaus(req, res) {
        taskLibrary.taskExpired({}, function (err, response) {
            ...
            query = { 'status': { "$in": status }, 'tasker': new mongoose.Types.ObjectId(req.body._id) }
            if (req.body.taskfrom) {
                var fromDate = req.body.taskfrom;
                query = { 'status': { "$in": status }, 'tasker': new mongoose.Types.ObjectId(req.body._id),'booking_information.booking_date': { '$gte': new Date(fromDate) }}
            }
            if (req.body.taskto) {
                var end = moment(new Date(req.body.taskto)).format('YYYY-MM-DD') + ' 23:59:59';
                var toDate = end;
                query = { 'status': { "$in": status }, 'tasker': new mongoose.Types.ObjectId(req.body._id),'booking_information.booking_date': { '$lte': new Date(toDate) }}
            }
            if (req.body.taskfrom && req.body.taskto) {
                query = { 'status': { "$in": status }, 'tasker': new mongoose.Types.ObjectId(req.body._id),'booking_information.booking_date': { '$gte': new Date(fromDate), '$lte': new Date(toDate) } }
            }
    ...
    controller.getSearchTaskDetailsByStaus = function getSearchTaskDetailsByStaus(req, res) {
        taskLibrary.taskExpired({}, function (err, response) {
            ...
            if (req.body.jobid.length > 4) {
                if (req.body.taskfrom && req.body.taskto) { // job id, from_date and to_date
                    match_query = { 'status': { "$in": status }, 'booking_id': req.body.jobid, 'updatedAt': { '$gte': new Date(fromDate), '$lte': new Date(toDate) }, 'tasker': new mongoose.Types.ObjectId(req.body._id) };
                } else if (req.body.taskfrom && !req.body.taskto) { // job id and from_date
                    match_query = { 'status': { "$in": status }, 'booking_id': req.body.jobid, 'updatedAt': { '$gte': new Date(fromDate) }, 'tasker': new mongoose.Types.ObjectId(req.body._id) };
                } else if (req.body.taskto && !req.body.taskfrom) { // job id and to_date
                    match_query = { 'status': { "$in": status }, 'booking_id': req.body.jobid, 'updatedAt': { '$lte': new Date(toDate) }, 'tasker': new mongoose.Types.ObjectId(req.body._id) };
                } else { // job id
                    match_query = { 'status': { "$in": status }, 'booking_id': req.body.jobid, 'tasker': new mongoose.Types.ObjectId(req.body._id) };
                }
            } else {
                if (req.body.taskfrom && req.body.taskto) { // from_date and to_date
                    match_query = { 'status': { "$in": status }, 'updatedAt': { '$gte': new Date(fromDate), '$lte': new Date(toDate) }, 'tasker': new mongoose.Types.ObjectId(req.body._id) };
                } else if (req.body.taskfrom && !req.body.taskto) { // from_date
                    match_query = { 'status': { "$in": status }, 'updatedAt': { '$gte': new Date(fromDate) }, 'tasker': new mongoose.Types.ObjectId(req.body._id) };
                } else if (req.body.taskto && !req.body.taskfrom) { // to_date
                    match_query = { 'status': { "$in": status }, 'updatedAt': { '$lte': new Date(toDate) }, 'tasker': new mongoose.Types.ObjectId(req.body._id) };
                }
            }
            ...
            if (err || !user) {
                ...
                async.waterfall([
                    ...
                    function (paymentgateway, transaction, settings, callback) {
                        db.GetDocument('emailtemplate', { name: { $in: ['PaymentDetailstoTasker', 'PaymentDetailstoUser'] }, 'status': { $ne: 0 } }, {}, {}, function (err, template) {
                            if (err || !template) {
                                data.response = 'Unable to get email template';
                                res.send(data);
                                            ...
                                            db.GetOneDocument('task', { _id: req.body.data.taskid }, {}, extension, function (err, task) {
                                                if (err || !task) {
                                                    ...
                                                    if (task.status == 3) {
                                                        ...
                                                        push.sendPushnotification(task.user._id, message, 'start_off', 'ANDROID', options, 'USER', function (err, response, body) { }, task.tasker.username);
                                                    } else if (task.status == 4) {
                                                        message = CONFIG.NOTIFICATION.PROVIDER_ARRIVED_ON_YOUR_PLACE;
                                                        var options = { 'job_id': task.booking_id, 'user_id': task.user._id };
                                                        push.sendPushnotification(task.user._id, message, 'provider_reached', 'ANDROID', options, 'USER', function (err, response, body) { }, task.tasker.username);
                                                    } else if (task.status == 5) {
                                                        message = CONFIG.NOTIFICATION.PROVIDER_STARTED_YOUR_JOB;
                                                        var options = { 'job_id': task.booking_id, 'user_id': task.user._id };
                                                        push.sendPushnotification(task.user._id, message, 'job_started', 'ANDROID', options, 'USER', function (err, response, body) { }, task.tasker.username);
            ...
            if (err || !paymentgateway.settings.live_secret_key || !paymentgateway.settings.sandbox_secret_key) {
                ...
                async.waterfall([
                    ...
                    function (settings, token, currencies, callback) {
                        var charge = {};
                        charge.amount = parseFloat(request.total_amount) * 100 * parseFloat(currencies.value);
                    ...
                    if (err || !docdata) {
                        ...
                        db.UpdateDocument('task', { _id: req.body.userid }, { status: req.body.taskstatus, usertaskcancellationreason: reason, 'history.job_cancellation_time': time, 'cancelledby': 'Tasker' }, function (err, result) {
                            if (err) {
                                ...
                                push.sendPushnotification(docdata.user._id, message, 'job_reassign', 'ANDROID', notifications, 'USER', function (err, response, body) { }, docdata.tasker.username);
                                res.send(result);
                            }
                        });
                        ...
                        if (err || !docdata) {
                            ...
                            db.UpdateDocument('task', { _id: req.body.userid }, { status: req.body.taskstatus, usertaskcancellationreason: reason, 'history.job_cancellation_time': time, 'cancelledby': 'User' }, function (err, result) {
                                if (err) {
                                    ...
                                    mailData1.language = docdata.tasker.language;
                                    mailData1.html = [];
                                    ...
                                    push.sendPushnotification(docdata.tasker._id, message, 'rejecting_task', 'ANDROID', notifications, 'PROVIDER', function (err, response, body) { }, docdata.user.username);
                                    res.send(result);
                                }
                            });
                        }
            ...
            if (err) {
                ...
                db.UpdateDocument('tasker', { _id: userid, 'taskerskills.childid': data.taskerskills.childid }, { $set: { "taskerskills.$": data.taskerskills } }, { multi: true }, function (err, result) {
                    if (err) {
                        ...
                        res.send({'result':result, 'status':'updated'});
                    }
                });
            } else {
                db.UpdateDocument('tasker', { _id: userid }, { $addToSet: { "taskerskills": data.taskerskills } }, { "multi": true }, function (err, result) {
                    if (err) {
						...
						res.send({'result':result, 'status':'added'});
    ...
    controller.deactivate = function deactivate(req, res) {
        db.GetDocument('users', { _id: req.body.userid }, { username: 1, email: 1, role: 1, language: 1 }, {}, function (err, docdata) {
            if (err) {
                res.send(err);
            } else {
                db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settingdata) {
                    if (err || !docdata) {
                        ...
                        mailData.language = docdata[0].language;
                        mailData.html = [];
    ...
    controller.deactivateTasker = function deactivateTasker(req, res) {
        db.GetDocument('tasker', { _id: req.body.userid }, { username: 1, email: 1, role: 1, language: 1 }, {}, function (err, docdata) {
            if (err) {
                res.send(err);
            } else {
                ...
                async.waterfall([
                    ...
                    function (token, task, tasker, currencies, callback) {
                        stripe.charges.create({
                            amount: parseFloat(task.invoice.amount.balance_amount) * 100 * parseFloat(currencies.value),
                            currency: currencies.code,
                            source: token.id,
                                                                        ...
                                                                        if (err) {
                                                                            ...
                                                                            db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settingdata) {
                                                                                if (err) {
                                                                                    ...
                                                                                    console.log("balance amount", invoice.amount.balance_amount);
                    ...
                    if (err || !currencies) { } else {
                        db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
                            ...
                            if (err || !settings) { } else {
                                ...
                                data.minimum_cost = req.query.symbol + " " + (task.invoice.amount.task_cost * req.query.value).toFixed(2);
                                ...
                                var lang = task.tasker.language || 'en';
                                i18n.setLocale(lang);
                                data.thanks_for_using_text = i18n.__('Thanks for using')
                                data.TOTAL_AMOUNT_text = i18n.__('TOTAL AMOUNT')
                                data.Tasker_Earnings_text = i18n.__('Tasker Earnings')
                                data.Task_amount_text = i18n.__('Task amount')
                                data.Admin_Fee_text = i18n.__('Admin Fee')
                                data.Minimum_Rate_text = i18n.__('Minimum Rate')
                                data.less_then_Hour_text = i18n.__('less then 1 Hour')
                                data.Total_text = i18n.__('Total')
                                data.TASK_DETAILS_text = i18n.__('TASK DETAILS')
                                data.OTHER_DETAILS_text = i18n.__('OTHER DETAILS')
                                data.Category_Name_text = i18n.__('Category Name')
                                data.User_Name_text = i18n.__('User Name')
                                data.Service_tax_text = i18n.__('Service tax')
                                data.Task_Address_text = i18n.__('Task Address')
                                data.Discount_Amount_text = i18n.__('Discount Amount')
                                data.Total_Hours_text = i18n.__('Total Hours')
                                data.Per_Hour_text = i18n.__('Per Hour')
                                data.Miscellaneou__text        = i18n.__('Miscellaneous Amount')
                                data.BOOKING_DETAILS_text = i18n.__('BOOKING DETAILS')
                                data.Booking_Id_text = i18n.__('Booking Id')
                                data.Booking_Email_id_text = i18n.__('Booking Email id')
                                data.Service_Address_text = i18n.__('Service Address')
                                data.Booking_Date_text = i18n.__('Booking Date')
                                data.Job_Completed_Date_text = i18n.__('Job Completed Date')
                                data.For_further_queries_please_write_to_text = i18n.__('For further queries, please write to')
                                data.This_is_an_electronically_generated_invoice_content_text = i18n.__('This is an electronically generated invoice content')
                    ...
                    if (err || !currencies) { } else {
                        db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
                            if (err || !settings) { } else {
                                ...
                                var lang = task.user.language || 'en';
                                i18n.setLocale(lang);
                                data.thanks_for_using_text = i18n.__('Thanks for using')
                                data.TOTAL_AMOUNT_text = i18n.__('TOTAL AMOUNT')
                                data.Task_amount_text = i18n.__('Task amount')
            ...
            if (err) {
                ...
                db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
                    if (err) {
                        ...
                        res.send({'canceldata': data, 'settings': settings.settings});
                    }
                });
            }
        });
    ...
    controller.saveaccountinfo = function (req, res) {
        ...
        db.UpdateDocument('tasker', { '_id': request.userid }, { 'banking': request.banking }, {}, function (err, response) {
            if (err || response.nModified == 0) {
                ...
                res.send(response);
                /*taskerLibrary.addStripeBankAccount(query, request.banking, function (err, striperes) {
                        if (err || !striperes) {
                            res.send(err);
                        } else {
                            res.send(response);
    ...
    controller.updateTaskerDocument = function (req, res) {
        var data = {};
        data.response = {};
        data.imagedata = {};
        ...
        if(req.files && req.files.length > 0) {
            ...
            db.UpdateDocument('tasker', { _id: req.body.tasker, 'doc._id': new mongoose.Types.ObjectId(req.body.doc_id) }, { $set: { "doc.$": files }, document_status: 2 }, { multi: true }, function (err, result) {
                if (err) {
                    res.send(err);
                } else {
                    var notifications = {};
    ...
    controller.taskerTaskDetails = async function (req, res) {  //async
        const userId = req.body.userId;
        var errors = [];
        req.checkBody('userId', res.__('UserId is Missing')).notEmpty();
        ...
        const taskProfile = [
            { '$match': { 'tasker': new mongoose.Types.ObjectId(userId), 'status': 5 } },
            {
                '$project': {
                    'booking_id': 1
                }
        ...
        try {
            const result = await db.GetAggregationDoc('task', taskProfile); //await removed
            if (result.length > 0) {
                res.send('Tasker Engaged');
            } else {
                res.send('Tasker Not Engaged');
            }
        } catch (e) {
            res.send({ 'status': 0, 'message': 'Unable to get profile details' });
        }
