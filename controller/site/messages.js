        ...
        async.parallel({
          ...
          if (err) {
            ...
            var condition = [
              ...
              { $unwind: "$tasker" },
              {
                $group: { _id: "$task", user: { $first: "$user" }, tasker: { $first: "$tasker" }, task: { $first: "$task" }, messages: { $push: { from: "$from", message: "$message", date: "$createdAt", status: "$status", user_status: "$user_status", tasker_status: "$tasker_status", datatype: "$datatype" } } }
              }
            ];
            db.GetAggregation('messages', condition, function (err, messages) {
              if (err || !messages[0]) {
                ...
                for (var i = 0; i < messages[0].messages.length; i++) {
                  ...
                  if (messages[0].messages[i].datatype == 'text') {
                    messages[0].messages[i].message = messages[0].messages[i].message;
                    messages[0].messages[i].messages =  messages[0].messages[i].message;
                  } else {
                    messages[0].messages[i].messages =  messages[0].messages[i].message;
                    messages[0].messages[i].message = result.settings.settings.site_url + messages[0].messages[i].message;
                  }
