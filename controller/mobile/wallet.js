...
module.exports = function (io,i18n) {
	...
	controller.payform = function (req, res) {
		...
		var data = {};
		data.status = '0';
		req.checkQuery('user_id', res.__('User ID is Required')).notEmpty();
			...
			if (err || !user) {
				...
				db.GetOneDocument('task', { '_id': request.task, 'status': 6 }, {}, options, function (err, task) {
					if (err || !task) {
						res.send({
							"status": "0",
							"message": res.__("This job informations is not available")
						});
					} else {
						async.waterfall([
							function (callback) {
							...
							if (err) {
								...
								db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
									if (err || !currencies) {
										res.send({
											"status": "0",
											"response": res.__('Unable to fetch currencies data')
										});
									}
									else {
										paypal.configure({
										...
										json.redirect_urls.return_url = "http://" + req.headers.host + "/mobile/app/payment/paypal-execute?task=" + task._id + "&transaction=" + transaction._id+ "&lang=" + lang;
										json.redirect_urls.cancel_url = "http://" + req.headers.host + "/checkout/payment/paypal/cancel?task=" + task._id + "&transaction=" + transaction._id+ "&lang=" + lang;
						...
						paypal.payment.execute(request.paymentId, { "payer_id": request.PayerID }, function (err, result) {
							if (err) {
								...
								if (result.state != 'approved') {
									data.response = res.__('Transaction Failed');
									res.redirect("mobile/payment-failed?lang=" + lang);
								} else {
														...
														db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
															if (err || !currencies) {
																...
																db.GetDocument('task', { 'booking_id': req.body.booking_id ,'invoice.coupon':{$exists: false} }, {}, {}, function (err, taskdata) {
																	...
																	res.send({ "status": "0", "response": res.__("Coupon is Already Applied")});
																	}
																	else {
																		db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
                                                                     if(err || !settings){
                                                                        res.send({ "status": "0", "response": res.__("Unable to get settings") });
                                                                     }
																	 else {
																		var invoice = {};
																		invoice.amount = {};
	...
	controller.updatewalletdatapaypal = function updatewalletdatapaypal(req, res) {
		...
		db.GetOneDocument('users', { '_id': request.user }, {}, {}, function (err, user) {
			if (err || !user) {
				res.send({
					"status": "0",
					"message": res.__("Invalid User ID, Please try Again Later")
				});
			} else {
				db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
				if(err){
